            ...
            for monstro in monstros:
                ...
                if tipo_c:
                    if isinstance(tipo_c, str):
                        dados["tipo_id"] = get_or_create(tipo_c.title(), "tipos", "tipo")
                    else:
                        ...
                        if not isinstance(tipo, dict):
                            ...
                            if subtipo:
                                subtipo.title()
                            dados["tipo_id"] = get_or_create(tipo.title(), "tipos", "tipo")
                ...
                with engine.begin() as conn:
                    ...
                    if deslocamento:
                        for i in deslocamento:
                            ...
                            if dis:
                                ...
                                des = get_or_create(i.title(), "deslocamentos", "deslocamento")
                    ...
                    if pericias:
                        for i in pericias:
                            peri = get_or_create(i.title(), "pericias", "pericia")
                            conn.execute(
                    ...
                    if resistencias:
                        for i in resistencias:
                            if isinstance(i, str):
                                dano = get_or_create(i.title(), "danos", "dano")
                                conn.execute(
                                ...
                                for j in i["resist"]:
                                    nao_magico = False
                                    if i.get("note") == "from nonmagical attacks":
                                        nao_magico = True
                                    if isinstance(j, str):
                                        dano = get_or_create(j.title(), "danos", "dano")
                                        conn.execute(
                                            insert(metadata.tables["resistencia_monstro"]).values({"dano_id": dano, "monstro_id": monstro_id, "nao_magico": nao_magico})
                                        )
                                    elif j.get("resist"):
                                        nao_magico = False
                                        if i.get("note") == "from nonmagical attacks":
                                            nao_magico = True
                                        for k in j["resist"]:
                                            dano = get_or_create(k.title(), "danos", "dano")
                                            conn.execute(
                                                insert(metadata.tables["resistencia_monstro"]).values({"dano_id": dano, "monstro_id": monstro_id, "nao_magico": nao_magico})
                                            )
                            ...
                            if isinstance(i, str):
                                ...
                                if aux:
                                    ...
                                    for j in aux:
                                        dano = get_or_create(j.title(), "danos", "dano")
                                        conn.execute(
                                            insert(metadata.tables["imunidade_dano_monstro"]).values({"dano_id": dano, "monstro_id": monstro_id, "nao_magico": nao_magico})
                                        )
                    ...
                    if imunidades_condicao:
                        for i in imunidades_condicao:
                            if isinstance(i, str):
                                condicao = get_or_create(i.title(), "condicoes", "condicao")
                                conn.execute(
                                ...
                                for j in imuni:
                                    condicao = get_or_create(j.title(), "condicoes", "condicao")
                                    conn.execute(
                                        insert(metadata.tables["imunidade_condicao_monstro"]).values({"condicao_id": condicao, "monstro_id": monstro_id})
                                    )
                    ...
                    if idiomas:
                        for i in idiomas:
                            if i.count(" ") == 0:
                                idi = get_or_create(i.title(), "idiomas", "idioma")
                                conn.execute(
                    ...
                    if sentidos:
                        for i in sentidos:
                            lista = i.split()
                            if len(lista) <= 3:
                                sen = get_or_create(lista[0].title(), "sentidos", "sentido")
                                conn.execute(
                    ...
                    if habitats:
                        for i in habitats:
                            lista = i.split()
                            hab = get_or_create(lista[0].replace(",", "").title(), "habitats", "habitat")
                            conn.execute(
                    ...
                    if tamanhos:
                        for i in tamanhos:
                            tam = get_or_create(tamanhos_aux[i], "tamanhos", "tamanho")
                            conn.execute(
